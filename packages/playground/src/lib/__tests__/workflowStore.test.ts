  ...
  describe("resolveRuntimeExpression", () => {
    ...
    describe("$inputs", () => {
      ...
      it("returns original expression if input not found", () => {
        expect(resolveRuntimeExpression("$inputs.notFound")).toBe(
          "$inputs.notFound",
        );
      });
    });
    ...
    describe("$steps", () => {
      ...
      it("resolves simple step result paths", () => {
        expect(
          resolveRuntimeExpression("$steps.getEvents.outputs.eventId"),
        ).toBe("123");
      });

      it("resolves array indexing", () => {
        expect(
          resolveRuntimeExpression("$steps.getEvents.data.events[0].id"),
        ).toBe("123");
        expect(
          resolveRuntimeExpression("$steps.getEvents.data.events[1].title"),
        ).toBe("Lunch");
      });

      it("resolves nested paths", () => {
        expect(
          resolveRuntimeExpression("$steps.getEvents.data.events[0].title"),
        ).toBe("Meeting");
      });

      it("returns original expression if step not found", () => {
        expect(resolveRuntimeExpression("$steps.notFound.data")).toBe(
          "$steps.notFound.data",
        );
      });

      it("returns original expression if path not found", () => {
        expect(resolveRuntimeExpression("$steps.getEvents.invalid.path")).toBe(
          "$steps.getEvents.invalid.path",
        );
      });
    });
    ...
    describe("$response", () => {
      ...
      it("resolves response body paths", () => {
        expect(
          resolveRuntimeExpression("$response.body#/calendar_event_id"),
        ).toBe("789");
        expect(
          resolveRuntimeExpression("$response.body#/details/location"),
        ).toBe("Office");
      });

      it("resolves response header paths", () => {
        expect(
          resolveRuntimeExpression("$response.headers#/content-type"),
        ).toBe("application/json");
      });

      it("returns original expression if path not found", () => {
        expect(resolveRuntimeExpression("$response.body#/invalid/path")).toBe(
    ...
    describe("$outputs", () => {
      ...
      it("returns original expression if output not found", () => {
        expect(resolveRuntimeExpression("$outputs.notFound")).toBe(
          "$outputs.notFound",
        );
      });
    });
