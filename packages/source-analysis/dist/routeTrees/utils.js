...
//   });
// }
export function getImportTypeDefinitionFileName(node, context) {
    ...
    if (nodeFileName !== declarationFileName) {
        ...
        if (compilerOptions.types) {
            const type = compilerOptions.types.find((name) => nodeFileName.indexOf(name));
            if (type) {
                const result = ts.resolveModuleName(type, nodeFileName, compilerOptions, host);
    ...
    const result = ts.resolveModuleName(text, nodeFileName, compilerOptions, host);
