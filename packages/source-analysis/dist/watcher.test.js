...
test("Initial files (added)", async () => {
    ...
    try {
        ...
        const basicFile = handler.mock.calls.find((call) => call[0].payload.fileName.endsWith("basic.ts"));
        expect(basicFile).toBeDefined();
        // Find index file event
        const indexFile = handler.mock.calls.find((call) => call[0].payload.fileName.endsWith("index.ts"));
        expect(indexFile).toBeDefined();
        // Find other file event
        const otherFile = handler.mock.calls.find((call) => call[0].payload.fileName.endsWith("other.ts"));
...
test("Empty folder", async () => {
    ...
    try {
        ...
        copyFileSync(path.join(simpleLocation, "index.ts"), path.join(location, "index.ts"));
...
function removeTypescriptFiles(location) {
