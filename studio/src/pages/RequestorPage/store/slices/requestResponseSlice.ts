...
import { addBaseUrl, removeBaseUrl, extractMatchedPathParams, extractPathParams, mapPathParamKey } from '../../reducer/reducer';
...
import { setBodyTypeReducer, updateContentTypeHeader } from '../../reducer/reducers';
import { RequestResponseSlice } from './types';
...
> = (set, get) => ({
  ...
  setServiceBaseUrl: (serviceBaseUrl) => set((state) => {
    ...
    state.path = addBaseUrl(serviceBaseUrl, state.path, { forceChangeHost: true });
  }),
  ...
  updatePath: (path) => set((state) => {
    ...
    state.activeHistoryResponseTraceId = state.selectedRoute === nextSelectedRoute
  ...
  updatePathParamValues: (pathParams) => set((state) => {
    state.pathParams = state.pathParams.map((pathParam: KeyValueParameter) => {
