...
export function createStructuredWorkflowPrompt(openApiSpec: string, workflowPlan: string) {
  return `Convert this workflow plan into a structured JSON format. Only output valid JSON - no explanations or additional text.
...
- You are returning JSON directly, not markdown. Do not prepend or append any text to your output. NO \`\`\`json backticks, no quotes, no nothing.
`;
}
