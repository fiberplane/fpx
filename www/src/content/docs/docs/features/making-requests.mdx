...
import CreateGoose from '@/assets/making-requests--create-goose.gif';
import CurlRequest from '@/assets/making-requests--curl.gif';
...
Say we have a route `POST /api/geese` that creates a goose. We can fill in the body with the data we want to send to the server as follows:
