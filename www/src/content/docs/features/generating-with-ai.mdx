...
In the Fiberplane Studio UI, Studio's AI can generate these fields for us. Once we've enabled AI Request Autofill (we'll tackle configuration [in a bit](#enabling-request-autofill)), we can click the Magic Wand icon on the requests page to generate some request data. Feel free to also use the shortcut `CMD + G`.
...
We can go ahead and click Send (or press `CMD + Enter`), and Studio will send the request to the API and create the goose!

{/* TODO: Add a screenshot of the response?? */}

In addition to generating the request body, Studio can also prefill data for:
...
If your LLM provider exposes an API that is compatible with OpenAI or Anthropic, you can use it.
...
## Another Example: Testing like a QA Engineer
