---
title: "Guide: Adding client-side logic to your Hono app"
...
\- though the same principles apply to other supported environments too.
...
component server-side with Hono.
If you're interested in client-side rendering only, check out the [example on
GitHub](https://github.com/oscarvz/cf-workers-hono-client-side).
</Aside>

You code editor might give you a hint that `document` is not defined. Given we
...
```tsx title="src/index.tsx" ins={2, 4, 8-24, 27} del={26}
...
### Adding build step and scripts

As we just added client-side logic. This needs to be built separately from the
Wrangler build step. We'll be using Vite and two plugins to facilitate both
build steps:
...
defining these bindings ourselves, the worker will automagically find the files
...
```tsx title="src/index.tsx" ins={12-17}
// ...
app.use(
	jsxRenderer(
		({ children }) => (
			<html lang="en">
				<head>
					...
					{import.meta.env.PROD ? (
						...
						<script src="/src/client/index.tsx" type="module" />
					)}
				</head>
				<body>
					<div id="root">{children}</div>
