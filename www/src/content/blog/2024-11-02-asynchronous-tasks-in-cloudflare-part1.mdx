...
All the asynchronous tasks (functions) are called sequentially. This blocking execution delays the response, which slows down the response to the user.
...
This visualization helps us understand the performance impact of sequential execution on our application:
...
This method leads to the parallel execution of both tasks, and the trace clearly shows how this improves our response time compared to the sequential approach:
...
Similar to the fire and forget trace, we see that the response is sent after the database insert, but the email is sent after the response is returned and starts sequentially after the database insert. The trace confirms our intended execution order while showing improved response times.
