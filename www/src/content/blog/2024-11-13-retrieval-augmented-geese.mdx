...
<Aside type="tip" title="Already familiar with embeddings?">
  Skip this section if you already know about vectors and embeddings and all
  that jazz.
</Aside>
...
Well, if all we have is the name of the color, there’s not much we can do. We’d just invent a new color and give it a new name, like `"purpleish blue"`.

With an RGB value, though, we can simply “add” some red:
...
Basically, instead of looking for exact matches, keyword matches, or fuzzy matches of a user's search query, we look for “semantically similar matches” based off of cosine distance.
...
Here, the `drizzleSql` helper is the `sql` helper provided by Drizzle, which has been renamed for clarity.
...
However, that doesn't mean that vector search is always the right tool. Any robust retrieval system should consider the trade-offs of vector search vs. keyword search, and likely combine the two.
...
- Add the ability to filter results by documentation section
...
There are a lot of libraries that try to abstract away the details of building RAG systems, but I think it's valuable to understand the core concepts.
For example, I've built small tools with Llamaindex and Langchain, and for simpler use cases, I found myself wading through documentation and GitHub issues more than I'd like.
(They can be very helpful for bigger applications, but for something this simple, I think the core concepts are valuable to implement and understand.)

While I was working on this, the two sticking points for me were:

- Pre-processing the documentation
- Crafting the similarity search query in Drizzle

The full code for everything in this post is available on GitHub, and you can adapt it for your own needs.

Check out the [HONC stack](https://honc.dev/) for more examples of building with Hono, Drizzle, Neon, and Cloudflare.
