...
[In Part 1](https://fiberplane.com/blog/asynchronous-tasks-in-cloudflare-part1/), we explored strategies for managing asynchronous tasks within a single Cloudflare Worker, from sequential execution to parallel background processing with `waitUntil()`.
While effective for simple cases, complex systems benefit from error handling, retries, and separation of concerns. These patterns are easier to achieve when we decompose asynchronous tasks into multiple workers.

In Part 2, we’ll distribute tasks across workers using Service Bindings, and then explore how to achieve enhanced fault tolerance with Cloudflare Queues.

Let’s start by breaking down the Marathon Sign-up Worker from Part 1!

## Example: A Marathon Registration API and an Email Worker using Cloudflare Service Bindings
...
## Example: A Marathon Registration API and an Email Worker using Cloudflare Queues

Remember how we couldn’t implement a reliable fire-and-forget solution, either within a single worker or through direct bindings?
This is because, in both cases (single worker and multiple workers), asynchronous tasks share the same thread.
The invocation pattern depends on the response from the other service, creating a dependency on the shared thread.

To address this, let’s further decouple the Sign-up Worker and the Email Worker, allowing both to operate more independently.
For example, what happens if the Email Worker fails? When implemented as a background task with `waitUntil(...)`, error handling becomes complicated and is only feasible across the entire call chain.

To decouple the workers and enable more granular error handling, we can use a message broker. Cloudflare provides [Queues](https://developers.cloudflare.com/queues/reference/how-queues-works/) to facilitate this approach.

![Example Queue and Workers architecture](@/assets/blog/2024-12-03-queue-architecture.png)

### Setting Up Cloudflare Queues
...
This way, the workers are decoupled and can operate independently.
...
### Sending Messages to the Queue (Producer)
...
In addition, messages can be sent by external applications to the Cloudflare queue, as shown in [this example using DAPR](https://github.com/diagrid-labs/dapr-cloudflare-queues).
This approach allows workers to remain solely within the internal Cloudflare network while enabling event-driven integration with services outside Cloudflare.

### Handling Messages from the Queue (Consumer)
...
Multiple workers can consume messages from the same queue, and messages are not tied to a specific worker. If there is more load, Cloudflare will automatically scale the workers to consume from the queue.

However, there may be cases where it's desirable to ensure only one worker processes the queue at a time, such as when maintaining state consistency or when tasks should not be executed concurrently.
...
By default, the Cloudflare queue tries to deliver a message three times. If the message is not delivered after three attempts, it is marked as failed.
You can modify this behavior in the consumer, where you can also define the maximum number of retries and set up a dead-letter queue for all failed messages.
...
When working with messages in a batch, each message can still be handled individually.
If individual acknowledgements are not defined, the entire batch is acknowledged or marked as failed (even if only one message fails).
Depending on the data, it may be necessary to handle the messages individually.

If the goal is to ensure that only failed messages are retried, individual messages within the batch must be acknowledged.
Otherwise, the entire batch will be marked for retry. This approach can help achieve [idempotent consumers](https://microservices.io/patterns/communication-style/idempotent-consumer.html).
...
With the attempt property and the ability to define `delaySeconds` in the retry method, a [backoff strategy](https://developers.cloudflare.com/queues/configuration/batching-retries/#apply-a-backoff-algorithm) can also be defined.

### Current limitations when developing locally

When developing and testing locally, you cannot run two independent workers and connect them to the same queue.
It is not possible to run the queue remotely and connect local workers to it.
...
Alternatively, you can test the workers and the queue in the Cloudflare network by deploying the logic to the network.
This approach involves testing in a production environment or creating a dedicated testing/stagining environment.

## Conclusion

Breaking up asynchronous tasks into multiple workers adds flexibility to the architecture.
It allows for the reusability of workers, makes them independently deployable, and enables the exclusion of certain workers from public-facing interfaces.

Additionally, when using Queues, workers can be loosely coupled, allowing for independent error and retry handling.
This aligns with an event-driven approach and also enables the connection of external systems to the queue, ensuring event-driven and serverless handling.
