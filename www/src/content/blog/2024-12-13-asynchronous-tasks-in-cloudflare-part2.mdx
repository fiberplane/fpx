...
Bindings can be setup quite easily in the `wrangler.toml` file.
...
The worker that is being bound to needs to use the `WorkerEntryPoint` class.
...
If you start both workers locally (in separate shells), you can see if your binding is working correctly.

![Console output for service binding](@/assets/blog/2024-12-13-workers-binding.png)

Now that the services are bound we can either use rpc to call a function directly in the other worker or use fetch to call an endpoint.
...
Fiberplane studio nicely outlines the invokation of the different workers in both cases.
