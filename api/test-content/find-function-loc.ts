...
async function findFunctionByDefinition(jsFilePath: string, functionDefinition: string): Promise<FunctionLocation | null> {
  try {
    const fileContent = await readFileAsync(jsFilePath, { encoding: 'utf-8' });
    const ast = parse(fileContent, { ecmaVersion: 'latest', locations: true, sourceType: 'module' });
...
findFunctionByDefinition(FILE_PATH, functionText).then(async loc => {
  ...
  const sourceFunctionStart = await findOriginalSource(FILE_PATH, functionStartLine, functionStartColumn);

  const functionEndLine = loc?.endLine ?? 0;
  const functionEndColumn = loc?.endColumn ?? 0;

  const sourceFunctionEnd = await findOriginalSource(FILE_PATH, functionEndLine, functionEndColumn);
...
// findOriginalSource('./function-log-index.js', 123, 34);  // Adjust line and column accordingly