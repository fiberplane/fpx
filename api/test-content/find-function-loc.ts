...
findFunctionByDefinition(FILE_PATH, functionText).then(async (loc) => {
	...
	const sourceFunctionStart = await findOriginalSource(
		FILE_PATH,
		functionStartLine,
		functionStartColumn,
	);
	...
	const sourceFunctionEnd = await findOriginalSource(
		FILE_PATH,
		functionEndLine,
		functionEndColumn,
	);
	...
	const sourceFunction = sourceContent
		...
		.filter(
			(_, i) =>
				i >= sourceFunctionStart.line - 1 && i <= sourceFunctionEnd.line - 1,
		)
		.join("\n");
